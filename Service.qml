...
Item {
  ...
  function retryNow(withRefused) {
    ...
    retryProcess.command = ["timeout", "300", "sh", "-c", '"$@" | head -c 200000', "sh", command].concat(withRefused ? ["retry", "--refused"] : ["retry"])
  ...
  // The shell never opens the journal itself. The bundled CLI pokes this
  // widget over IPC after every capture, and a slow poll covers a CLI that
  // does not.
  Timer {
    interval: 30000
    repeat: true
  ...
  Process {
    id: queueProcess
    running: false
    command: ["timeout", "15", "sh", "-c", '"$@" | head -c 2000000', "sh", root.command, "queue", "--json"]
  ...
  Process {
    id: journalProcess
    running: false
    command: ["timeout", "15", "sh", "-c", '"$@" | head -c 2000000', "sh", root.command, "journal", "--json", "-n", "60"]
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (root.reachable && root.counts.waiting > 0 && !retryProcess.running) {
        retryProcess.command = ["timeout", "300", "sh", "-c", '"$@" | head -c 200000', "sh", root.command, "retry", "--quiet"]
        retryProcess.running = true
      }
    }
  }
